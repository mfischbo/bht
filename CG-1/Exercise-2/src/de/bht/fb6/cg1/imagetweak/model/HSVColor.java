...
public class HSVColor {
	...
	public void setParameters(final int[] parameters) throws IllegalArgumentException {
		...
		if (parameters[0] < 0 || parameters[0] > 255)
			throw new IllegalArgumentException("Value for the component 'hue="+parameters[0]+"' is out of the range [0;255]");
		if (parameters[1] < 0 || parameters[1] > 255)
			throw new IllegalArgumentException("Value for the component 'saturation="+parameters[1]+"' is out of the range [0;255]");
		if (parameters[2] < 0 || parameters[2] > 255)
			throw new IllegalArgumentException("Value for the component 'value="+parameters[2]+"' is out of the range [0;255]");
	...
	public int[] toRGB() {
		...
		final double qH = (this.hue * 360.0d) / 255.0d;
		final double qS = this.saturation / 255.0d;
		final double qV = this.value / 255.0d;
		
		if (qH < 120) {
			rgb[0] = (120.0d - qH) / 60.0d;
			rgb[1] = qH / 60.0d;
			rgb[2] = 0.0d;
		} else if (qH < 240) {
			rgb[0] = 0.0;
			rgb[1] = (240.0d - qH) / 60.0d;
			rgb[2] = (qH - 120) / 60.0d;
		} else {
			rgb[0] = (qH - 240.0d) / 60.0d;
			rgb[1] = 0.0;
			rgb[2] = (360.0d - qH) / 60.0d;
		}
		...
		ret[0] = ((1 - qS + qS * rgb[0]) * qV) * 255.0d;
		ret[1] = ((1 - qS + qS * rgb[1]) * qV) * 255.0d;
		ret[2] = ((1 - qS + qS * rgb[2]) * qV) * 255.0d;
