 ...
 * After parsing the result is returned as {@link HttpRequest}.
 * 
 * @author M. Fischboeck
 *
 */
public class HttpParser {
	 ...
	 */
	public static HttpRequest parse(String requestString) throws Exception {
		...
		if (tokens[0].equalsIgnoreCase("GET"))
			retval.setMethod(HttpMethod.GET);
		
		if (tokens[0].equalsIgnoreCase("POST"))
			retval.setMethod(HttpMethod.POST);
		
		if (tokens[0].equalsIgnoreCase("DELETE"))
			retval.setMethod(HttpMethod.DELETE);
		
		if (tokens[0].equalsIgnoreCase("PUT"))
			retval.setMethod(HttpMethod.PUT);
